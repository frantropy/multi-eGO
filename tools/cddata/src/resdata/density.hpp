...
{
  std::size_t same_mutex_index = resdata::indexing::mutex_access(mol_id_[i], a_i, a_j, natmol2_);
...
void inter_mol_same_routine(
  int i, std::size_t a_i, std::size_t a_j, float dx2, float weight, 
...
)
{
  ...
  std::size_t same_access_index = resdata::indexing::offset_same(mol_id_[i], i, a_i, a_j, natmol2_);
  std::size_t same_mutex_index = resdata::indexing::mutex_access(mol_id_[i], a_i, a_j, natmol2_);
  std::unique_lock lock(frame_same_mutex_[mol_id_[i]][same_mutex_index]);
...
void inter_mol_cross_routine(
  int i, int j, std::size_t a_i, std::size_t a_j, float dx2, float weight,
...
{
  float dist = std::sqrt(dx2);
  std::size_t cross_access_index = resdata::indexing::offset_cross(mol_id_[i], mol_id_[j], i, j, a_i, a_j, natmol2_);
  std::size_t cross_mutex_index = resdata::indexing::mutex_access(mol_id_[j], a_i, a_j, natmol2_);
